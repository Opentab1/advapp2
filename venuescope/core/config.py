...
class DrinkCountRules:
    ...
    serve_dwell_frames:      int   = 3    # min frames bartender must dwell on customer side (bilateral crossing)
    serve_cooldown_seconds:  float = 30.0 # min seconds between serves per station (converted to frames at runtime)
    serve_cooldown_frames:   int   = 180  # computed by engine from serve_cooldown_seconds × effective_fps
