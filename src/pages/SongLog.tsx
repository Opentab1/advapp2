...
export function SongLog() {
  ...
  const EmptyState = ({ icon: Icon, title, message }: { icon: any; title: string; message: string }) => (
    <div className="flex flex-col items-center justify-center py-8 text-center">
      <Icon className="w-12 h-12 text-warm-600 mb-3" />
      <p className="text-warm-400 font-medium">{title}</p>
      <p className="text-sm text-warm-500 mt-1">{message}</p>
    </div>
  );
  ...
  return (
    ...
    <div className="max-w-7xl mx-auto">
      ...
      >
        ...
        <div className="flex items-center gap-2 mb-6 text-sm text-warm-400">
          <Calendar className="w-4 h-4" />
          <span>Showing analytics for: <span className="text-primary font-medium">{getTimeRangeLabel(timeRange)}</span></span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
            ...
            <div className="space-y-3 max-h-[400px] overflow-y-auto custom-scrollbar">
              {analyticsLoading && !highestPerforming.length ? (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        ...
                        <span className="text-xs text-warm-500">avg {song.avgCrowdSize} people</span>
                      </div>
                    </div>
                    <div className="text-right">
                      <div className={`text-lg font-bold ${song.retentionRate >= 100 ? 'text-emerald-400' : 'text-amber-400'}`}>
                        {song.retentionRate >= 100 ? '+' : ''}{(song.retentionRate - 100).toFixed(1)}%
                      </div>
                      <div className="text-xs text-warm-500">retention</div>
                      <div className="text-xs text-warm-600 mt-0.5">
                        {song.avgExitRate.toFixed(1)} exit rate
                      </div>
                    </div>
                  </motion.div>
                ))
                ...
                <EmptyState 
                  ...
                  message="Play more songs to see which ones keep people"
                />
              )}
            </div>
          </motion.div>
          ...
          >
                  ...
                  {showExportMenu && (
                    ...
                    >
                      <button
                        ...
                        className="flex items-center gap-2 px-4 py-2 text-sm text-white hover:bg-warm-700 w-full"
                      >
            ...
            <p className="text-xs text-warm-400 mb-4">Your venue's crowd-favorite tracks, ready to export</p>
            ...
            <div className="space-y-2 max-h-[400px] overflow-y-auto custom-scrollbar">
              {analyticsLoading && !topPerformersPlaylist.length ? (
                <div className="text-center py-8">
                  ...
                  <p className="text-sm text-warm-400">Building your playlist...</p>
                </div>
                ...
                topPerformersPlaylist.map((song, index) => (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0">
                      <span className="text-xs text-warm-500">{song.plays}x</span>
                      <Zap className="w-3 h-3 text-green-400" />
                      <span className={`text-xs ${song.retentionRate >= 100 ? 'text-green-400' : 'text-amber-400'}`}>
                        {song.retentionRate >= 100 ? '+' : ''}{(song.retentionRate - 100).toFixed(1)}%
                      </span>
                    </div>
                  </motion.div>
            ...
            {topPerformersPlaylist.length > 0 && (
              ...
              >
                <div className="text-center text-xs text-warm-400">
                  ...
                  Sorted by retention rate ({timeRange})
                </div>
              </motion.div>
            )}
          </motion.div>
            ...
            <div className="space-y-3 max-h-[350px] overflow-y-auto custom-scrollbar">
              {analyticsLoading && !genreStats.length ? (
                <div className="text-center py-8">
                  ...
                  <p className="text-xs text-warm-400">Analyzing genres...</p>
                </div>
              ) : genreStats.length > 0 ? (
          ...
          >
            ...
            <div className="space-y-3">
              {topSongs.length > 0 ? topSongs.slice(0, 8).map((song, index) => (
                <motion.div
                  key={`${song.song}-${song.artist}`}
                  className="flex items-center justify-between p-3 rounded-lg bg-warm-800 hover:bg-warm-700 transition-colors"
                  initial={{ opacity: 0, x: -10 }}
                  animate={{ opacity: 1, x: 0 }}
                  transition={{ delay: 0.2 + index * 0.05 }}
                >
                  <div className="flex items-center gap-3 min-w-0">
                    <span className="text-primary font-bold text-lg w-5">{index + 1}</span>
                    <div className="min-w-0">
                      <div className="text-sm font-medium text-white truncate">{song.song}</div>
                      <div className="text-xs text-warm-400 truncate">{song.artist}</div>
                    </div>
                  </div>
                  <div className="text-primary font-bold flex-shrink-0">{song.plays}x</div>
                </motion.div>
              )) : (
                <EmptyState 
                  icon={TrendingUp}
            ...
            <div className="space-y-2 max-h-[350px] overflow-y-auto custom-scrollbar">
              {loading ? (
                <div className="text-center py-12">
                  ...
                  <p className="text-warm-400 text-sm">Loading songs...</p>
                </div>
              ) : songs.length > 0 ? (
                songs.slice(0, 15).map((song, index) => (
                  <motion.div
                    key={song.id}
                    className="flex items-center gap-3 p-2 rounded-lg bg-warm-800 hover:bg-warm-700 transition-colors"
                    initial={{ opacity: 0, y: 10 }}
                    animate={{ opacity: 1, y: 0 }}
          ...
          >
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              ...
              <div className="p-4 rounded-lg bg-warm-800">
                ...
                <div className="text-sm text-warm-400">Genres</div>
              </div>
              <div className="p-4 rounded-lg bg-warm-800">
                <div className="text-2xl font-bold text-emerald-400">{topSongs.length}</div>
                <div className="text-sm text-warm-400">Unique Songs</div>
              </div>
              <div className="p-4 rounded-lg bg-warm-800">
                <div className="text-2xl font-bold text-cyan-400">
                  {highestPerforming.length > 0 
                    ? `${((highestPerforming[0]?.retentionRate || 100) - 100).toFixed(1)}%`
                    : '—'
                  }
                </div>
                <div className="text-sm text-warm-400">Best Retention</div>
              </div>
            </div>
          </motion.div>
        )}
