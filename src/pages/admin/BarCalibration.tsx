/**
 * BarCalibration — Admin tool to auto-calibrate bar line position.
 *
 * Upload a 5-10 min drink clip + enter actual drink count.
 * The droplet runs a sweep of bar line Y positions and customer_side values,
 * finds the best match, and auto-writes bar_main.json for that venue.
 */
...
export function BarCalibration() {
  ...
  // Sync venue selector with context
  useEffect(() => {
  ...
  const handleRun = async () => {
    ...
    if (!videoFile)     { setError('Upload a video clip first.'); return; }
    if (!actualCount || parseInt(actualCount) < 1) {
      setError('Enter the actual number of drinks served in the clip.');
      return;
    }
    if (!CALIBRATION_URL) {
      setError('VITE_CALIBRATION_URL is not configured. Set it in Amplify env vars.');
      return;
    }
    ...
    try {
      const res = await fetch(`${CALIBRATION_URL}/calibrate`, {
        method: 'POST',
        body:   form,
      });
    ...
    <div className="p-6 max-w-3xl mx-auto space-y-6">
      ...
      <div>
        ...
        <p className="text-gray-400 text-sm mt-1">
          Upload a 5–10 min drink clip + enter actual count.
          The system sweeps 16 bar line configurations and writes the best one automatically.
        </p>
      </div>
      ...
      <div className="flex items-start gap-3 p-4 rounded-xl bg-blue-500/10 border border-blue-500/25">
        ...
        <div className="text-xs text-blue-300 space-y-1">
          ...
          <p>Only the first 5 minutes of the clip are processed. Bar config is auto-saved on completion.</p>
        </div>
      </div>
      ...
      <div className="glass-card rounded-xl p-5 space-y-4">
        ...
        <div>
          ...
          <select
            value={venueId}
            onChange={e => { setVenueId(e.target.value); setSelectedVenueId(e.target.value); }}
      ...
      {/* Progress */}
      <AnimatePresence>
        {job && (job.status === 'running' || isDone || isFailed) && (
          <motion.div
              ...
              <div className="space-y-4 pt-2">
                ...
                <div className="p-4 rounded-xl bg-green-500/10 border border-green-500/25">
                  <p className="text-xs text-green-400 font-medium mb-2 uppercase tracking-wide">
                    Best configuration — auto-applied
                  </p>
                      ...
                      <table className="w-full text-xs">
                        ...
                        <tbody>
                          {rows.map((r, i) => (
                            <tr
                              key={i}
                              className={`border-b border-white/5 ${i === 0 ? 'text-green-400' : 'text-gray-300'}`}
                            >
                ...
                {job.result?.bar_config_path && (
                  <p className="text-xs text-gray-500">
                    Config saved: <code className="text-amber-400">{job.result.bar_config_path}</code>
                  </p>
                )}
              </div>
            )}
      ...
      {/* Setup guide if no URL configured */}
      {!CALIBRATION_URL && (
        <div className="flex items-start gap-3 p-4 rounded-xl bg-amber-500/10 border border-amber-500/25">
          ...
          <div className="text-xs text-amber-300 space-y-1">
            ...
            <p>Add it in Amplify → App Settings → Environment Variables:</p>
            <code className="block bg-black/40 px-2 py-1 rounded text-amber-400 mt-1">
              VITE_CALIBRATION_URL=https://137-184-61-178.sslip.io:8502
            </code>
            <p className="mt-1 text-amber-400/70">Then redeploy the Amplify app.</p>
          </div>
        </div>
      )}
    </div>
...
function AccuracyBadge({ pct }: { pct: number }) {
  const color = pct >= 90 ? 'text-green-400'
