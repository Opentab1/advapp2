...
export function VenuesManagement() {
  ...
  // Drill-down to a single venue's full dashboard. When set, the page renders
  // VenueDetail (cameras + droplet + ops + recent jobs) instead of the list.
  ...
  // Drill-down: render the dedicated venue page when a card was clicked.
  // Lookup happens off the cached venues list so VenueDetail starts with the
  // same data the card was rendering — no extra fetch round-trip.
  if (selectedVenueId) {
    const v = venues.find(x => x.venueId === selectedVenueId);
    if (v) {
      return (
        <VenueDetail
          venue={v}
          displayName={displaySettings[v.venueId]?.displayName}
          onBack={() => setSelectedVenueId(null)}
        />
      );
    }
    // Selected venue not in list (deleted? race condition?) — clear and fall through.
              ...
              >
                <div className="flex items-start justify-between mb-4">
                  <div className="flex-1">
                    {/* Clicking the name drills into the dedicated venue page
                        (cameras + droplet + ops + recent jobs in one view). The
                        action buttons below stay independent so a misclick on
                        Edit/Delete doesn't navigate away. */}
                    <div className="flex items-center gap-3 mb-2 flex-wrap">
                      <button
                        onClick={() => setSelectedVenueId(venue.venueId)}
                        className="text-xl font-bold text-white hover:text-purple-400 hover:underline underline-offset-4 transition-colors text-left"
                        title="Open full venue dashboard"
                      >
