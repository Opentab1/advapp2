  ...
  if (!thisWeek) {
    return (
      <div className="bg-warm-800 rounded-2xl border border-warm-700 p-6 text-center transition-colors">
        <p className="text-warm-400">Not enough data for weekly comparison</p>
      </div>
    );
  }
    ...
    >
      ...
      <div className="px-4 py-3 bg-warm-700/50 border-b border-warm-700">
        <div className="flex items-center justify-between">
          <h3 className="font-semibold text-warm-100">Weekly Comparison</h3>
          <div className="flex items-center gap-2 text-xs text-warm-400">
            <span className="px-2 py-0.5 bg-primary/20 text-primary rounded font-medium">This Week</span>
            <span>vs</span>
            <span className="px-2 py-0.5 bg-warm-600 rounded">Last Week</span>
          </div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="p-3 rounded-xl bg-green-900/20 border border-green-900/30 transition-colors">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-xs font-medium text-green-400">Best Hour</span>
            </div>
            {thisWeek.bestHour ? (
              <>
      ...
      {thisWeek.dailyScores.length > 0 && (
        <div className="px-4 pb-4">
          <p className="text-xs text-warm-400 mb-2">Daily Pulse Scores</p>
            ...
            {thisWeek.dailyScores.map((score, i) => {
              ...
              return (
                <div key={i} className="flex-1 flex flex-col items-center gap-1">
                  ...
                  <span className={`text-[10px] ${isToday ? 'font-bold text-primary' : 'text-warm-500'}`}>
                    {['S', 'M', 'T', 'W', 'T', 'F', 'S'][i]}
                  </span>
                </div>
              );
