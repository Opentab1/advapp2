...
if mode == "drink_count":
    ...
    # ── Bar-unattended alerts ──────────────────────────────────────────────
    ua_events = summary.get("bar_unattended_events", [])
    if ua_events:
        st.divider()
        st.subheader("🚨 Bar Unattended Events")
        st.caption("Bar zone had no assigned staff for the configured alert threshold.")
        for ua in ua_events:
            mins, secs = divmod(int(ua.get("t_sec", 0)), 60)
            st.markdown(
                f'<div class="theft-flag">⚠️ Bar unattended at '
                f'<strong>{mins:02d}:{secs:02d}</strong> '
                f'({ua.get("duration_frames","?")} frames with no staff detected)</div>',
                unsafe_allow_html=True)
    elif mode == "drink_count":
        st.markdown('<div class="theft-ok">✅ No bar-unattended events detected</div>',
                    unsafe_allow_html=True)

    # ── Customer queue depth chart ──────────────────────────────────────────
    queue_log = summary.get("queue_log", [])
    if queue_log:
        st.divider()
        st.subheader("👥 Customer Queue Depth")
        st.caption("Number of customers on the customer side of the bar over time.")
        qdf = pd.DataFrame(queue_log).set_index("t_sec")
        st.line_chart(qdf)
    ...
    if evf.exists():
        try:
            _all_evs = pd.read_csv(evf)
            _serve_evs = _all_evs[_all_evs["event_type"] == "drink_serve"].copy() \
                if "event_type" in _all_evs.columns else _all_evs.copy()

            def _fmt_time(t):
                t = int(float(t))
                return f"{t//3600:02d}:{(t%3600)//60:02d}:{t%60:02d}" if t >= 3600 \
            ...
            # Confirmed serves (review == False or missing)
            _confirmed = _serve_evs[_serve_evs.get("review", pd.Series([False]*len(_serve_evs))) != True] \
                if "review" in _serve_evs.columns \
                else _serve_evs
            # Low-confidence / review bucket
            _low_conf = _serve_evs[_serve_evs["review"] == True] \
                if "review" in _serve_evs.columns else pd.DataFrame()

            if not _confirmed.empty or not _low_conf.empty:
                st.divider()
                st.subheader("📋 Detection Log")

            if not _confirmed.empty:
                _log_rows = []
                for _, _ev in _confirmed.iterrows():
                    _sc = float(_ev.get("serve_score", 0) or 0)
                    _log_rows.append({
                        "Time":       _fmt_time(_ev.get("t_sec", 0)),
                        "Bartender":  str(_ev.get("bartender", "")) or "unassigned",
                        "Drink Type": str(_ev.get("drink_type", "unknown")).replace("_", " ").title(),
                        "Method":     "glass" if str(_ev.get("detection_method","")).startswith("glass") else "body",
                        "Confidence": f"{_conf_color(_sc)} {_conf_pct(_sc)}",
                    })
                st.caption(f"**{len(_log_rows)} confirmed detection{'s' if len(_log_rows)!=1 else ''}** — above confidence threshold")
                st.dataframe(pd.DataFrame(_log_rows), use_container_width=True, hide_index=True)

            if not _low_conf.empty:
                st.markdown("---")
                st.markdown(f"**⚠️ Low Confidence Detections** — {len(_low_conf)} event{'s' if len(_low_conf)!=1 else ''} below threshold (not counted)")
                st.caption("These were detected but scored too low to confirm. Review manually to approve or dismiss.")
                _lc_rows = []
                for _, _ev in _low_conf.iterrows():
                    _sc = float(_ev.get("serve_score", 0) or 0)
                    _lc_rows.append({
                        "Time":       _fmt_time(_ev.get("t_sec", 0)),
                        "Bartender":  str(_ev.get("bartender", "")) or "unassigned",
                        "Drink Type": str(_ev.get("drink_type", "unknown")).replace("_", " ").title(),
                        "Confidence": f"{_conf_color(_sc)} {_conf_pct(_sc)}",
                        "Reason":     str(_ev.get("review_reason", "")) or "low score",
                    })
                st.dataframe(pd.DataFrame(_lc_rows), use_container_width=True, hide_index=True)
...
# ─────────────────────────────────────────────────────────────────────────────
    ...
    ts = summary.get("table_service", {})
    tables_ts  = ts.get("tables", {})
    total_vis  = ts.get("total_service_visits", 0)
    avg_rt     = ts.get("avg_response_time_min")
    alerts     = ts.get("unvisited_alerts", [])
    alert_min  = ts.get("alert_interval_min", 15)

    hc1, hc2, hc3 = st.columns(3)
    with hc1:
        st.markdown(f'<div class="hero-number">{total_vis}</div>'
                    f'<div class="hero-label">Total Service Visits</div>', unsafe_allow_html=True)
    with hc2:
        rt_val = f"{avg_rt:.1f} min" if avg_rt is not None else "—"
        rt_color = "#ef4444" if (avg_rt or 0) > 15 else "#f97316" if (avg_rt or 0) > 8 else "#22c55e"
        st.markdown(f'<div class="hero-number" style="color:{rt_color}">{rt_val}</div>'
                    f'<div class="hero-label">Avg Response Time</div>', unsafe_allow_html=True)
    with hc3:
        al_color = "#ef4444" if alerts else "#22c55e"
        st.markdown(f'<div class="hero-number" style="color:{al_color}">{len(alerts)}</div>'
                    f'<div class="hero-label">Unvisited Table Alerts</div>', unsafe_allow_html=True)

    if alerts:
        st.divider()
        for a in alerts:
            st.markdown(
                f'<div class="theft-flag">🚨 <strong>{a["table_label"]}</strong> has not been visited '
                f'in <strong>{a["minutes_since_last_visit"]:.0f} min</strong> '
                f'(threshold: {alert_min} min)</div>', unsafe_allow_html=True)
    else:
        st.markdown(
            f'<div class="theft-ok">✅ All tables visited within the {alert_min}-minute target</div>',
            unsafe_allow_html=True)

    if tables_ts:
        st.divider()
        st.subheader("🛎️ Per-Table Breakdown")
        rows = []
        for tid, d in tables_ts.items():
            rt = d.get("avg_response_min")
            rows.append({
                "Table":               d.get("label", tid),
                "Service Visits":      d.get("service_visit_count", 0),
                "Avg Dwell (s)":       d.get("avg_dwell_sec", 0),
                "Avg Response (min)":  rt if rt is not None else "—",
                "Max Response (min)":  d.get("max_response_min") or "—",
                "Last Visited (min ago)": d.get("minutes_since_last_visit", "—"),
            })
        tsdf = pd.DataFrame(rows).sort_values("Service Visits", ascending=False)
        st.dataframe(tsdf, use_container_width=True, hide_index=True)

        # Response time bar chart (exclude tables with no visits)
        rt_rows = [{"Table": r["Table"], "Avg Response (min)": r["Avg Response (min)"]}
                   for r in rows if isinstance(r["Avg Response (min)"], (int, float))]
        if rt_rows:
            st.divider()
            st.subheader("⏱ Response Time by Table")
            st.caption("Lower is better — target < 8 min for bar environments.")
            st.bar_chart(pd.DataFrame(rt_rows).set_index("Table"))

        # Visit timeline
        _ts_all = []
        for tid, d in tables_ts.items():
            for ts_t in d.get("visit_timestamps", []):
                _ts_all.append({"Table": d.get("label", tid), "Time (s)": ts_t})
        if _ts_all:
            st.divider()
            st.subheader("📅 Visit Timeline")
            _dur = summary.get("video_seconds", 1)
            _bin = max(30, int(_dur / 20))
            vis_df = pd.DataFrame(_ts_all)
            vis_df["Bin"] = (vis_df["Time (s)"] // _bin * _bin).astype(int)
            vis_df["Bin_label"] = vis_df["Bin"].apply(lambda s: f"{s//60}:{s%60:02d}")
            pivot = vis_df.pivot_table(index="Bin_label", columns="Table",
                                       values="Time (s)", aggfunc="count", fill_value=0)
            bin_order = sorted(vis_df["Bin"].unique())
            pivot = pivot.reindex([f"{b//60}:{b%60:02d}" for b in bin_order]).fillna(0)
            st.bar_chart(pivot, use_container_width=True)
